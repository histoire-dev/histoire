...
export function tailwindTokens(options: TailwindTokensOptions = {}): Plugin {
  ...
  async function generate(api: PluginApiBase) {
    try {
      ...
      /**
       * `resolveConfig` was removed in Tailwind v4
       * - so we can't as easily get all of the config/theme variables via javascript anymore
       *
       * we could reference the "real css variables" in the story template, but that would break
       * for all users who have customized their tailwind config
       * (especially if they've added custom names/spacings to the theme)
       *
       * Relevant Issue:
      ...
      await api.fs.writeFile(storyFile, storyTemplate(tailwindConfig))
...
function storyTemplate(sourceTailwindConfig: any) {
  console.warn('story template: tailwindConfig', sourceTailwindConfig)

  const tailwindConfig = {
    prefix: 'htw:',
    theme: {
      borderRadius: {
        ...sourceTailwindConfig.theme.borderRadius,
        ...sourceTailwindConfig.theme.extend.borderRadius,
      },
    },
  }
...
// function _storyTemplate(tailwindConfig: any) {
