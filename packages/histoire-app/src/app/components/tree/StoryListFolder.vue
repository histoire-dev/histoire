...
<template>
  ...
  >
    <div
      role="button"
      tabindex="0"
      class="histoire-story-list-folder-button htw:px-0.5 htw:py-2 htw:md:py-1.5 htw:mx-1 htw:rounded-sm htw:hover:bg-primary-100 htw:dark:hover:bg-primary-900 htw:cursor-pointer htw:select-none htw:flex"
      @click="toggleOpen"
      @keyup.enter="toggleOpen"
      @keyup.space="toggleOpen"
    >
      <span class="bind-tree-padding htw:flex htw:items-center htw:gap-2 htw:min-w-0">
        <span class="htw:flex htw:flex-none htw:items-center htw:opacity-30 htw:[.histoire-story-list-folder-button:hover_&]:opacity-100 htw:ml-4 htw:w-4 htw:h-4 htw:rounded-sm htw:border htw:border-gray-500/40">
          <Icon
