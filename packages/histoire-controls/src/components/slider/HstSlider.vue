  ...
  >
    <div class="htw:relative htw:w-full htw:flex htw:items-center">
      <div class="htw:absolute htw:inset-0 htw:flex htw:items-center">
        <div class="htw:border htw:border-black/25 htw:dark:border-white/25 htw:h-1 htw:w-full htw:rounded-full" />
      </div>
      <input
        ref="input"
...
.htw:range-input {
  &::-webkit-slider-thumb {
    @apply htw:appearance-none htw:h-3 htw:w-3 htw:bg-white htw:dark:bg-gray-700 htw:border htw:border-solid htw:border-black/25 htw:dark:border-white/25 htw:rounded-full;
  }
