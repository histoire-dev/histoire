...
export default {
  ...
  async serverAction (params) {
    ...
    const componentContent = await fs.promises.readFile(params.component, 'utf-8')
    const isTs = componentContent.includes('lang="ts"')

    const component = path.basename(params.component)
