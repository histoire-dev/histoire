...
<template>
  ...
  >
    <div
      ...
      :class="{
        'htw:cursor-pointer htw:hover:text-primary-500': histoireConfig.responsivePresets?.length,
      }"
    >
      <Icon
        icon="carbon:devices"
      ...
      <div class="htw:flex htw:flex-col htw:items-stretch">
        ...
        <div class="htw:flex htw:items-center htw:gap-2 htw:px-4 htw:py-3">
          <input
            ...
            type="number"
            class="htw:bg-transparent htw:border htw:border-gray-200 htw:dark:border-gray-850 htw:rounded htw:w-20 htw:opacity-50 htw:focus:opacity-100 htw:flex-1 htw:min-w-0"
            step="16"
            placeholder="Auto"
          >
