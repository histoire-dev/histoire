          ...
          plugins: [
            {
              ...
              transform(code, id) {
                ...
                if ((this.meta as any).histoire?.isCollecting && id.endsWith('.vue')) {
                  return `const _stubComponent = (name) => ['Story','story','Variant','variant'].includes(name) ? _resolveComponent(name) : ({ render: () => null });${code?.replaceAll('_resolveComponent(', '_stubComponent(') ?? ''}`
                }
              },
            },
          ],
