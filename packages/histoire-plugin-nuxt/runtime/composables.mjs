...
export function useNuxtApp() {
  ...
  const _options = defu(options, { nuxtAppSettings: {mock: {} }});

  const settings = _options.nuxtAppSettings;
  if (settings.include && Array.isArray(settings.include)) {
    settings.include = settings.include.map(parseOptionalRegexp);
  } else if (settings.exclude && Array.isArray(settings.exclude)) {
    settings.exclude = settings.exclude.map(parseOptionalRegexp);
  }
...
function parseOptionalRegexp(value) {
